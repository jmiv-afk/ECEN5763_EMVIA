...
// The active transformation in the OpenCV output window
//   displays canny if true, sobel if false
bool is_canny_active = true;
...
static void process_image(const void *p, int size)
{
    ...
    else if(fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_YUYV)
    {
       ...
       if (is_canny_active) {
        LOGSYS("E2P4-Can, %u, %6.2f", framecnt, get_time_msec()-prev_time); 
       } else {
        LOGSYS("E2P4-Sob, %u, %6.2f", framecnt, get_time_msec()-prev_time);
       }
       prev_time = get_time_msec();
       imshow( timg_window_name, timg_grad);

       char user_char = (char) waitKey(10);
       if (user_char == 'C' || user_char == 'c') {
         is_canny_active = true;
       } else if ( user_char == 'S' || user_char == 's') {
         is_canny_active = false;
       } else if ( user_char == 27 || user_char == 'Q' || user_char == 'q') {
         exit(0);
       }

       // Commented out for exercise 2
