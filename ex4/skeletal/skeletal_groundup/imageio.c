...
ppm_img_t* copy_ppm(ppm_img_t* old) {
  ...
  ppm_img_t *new = new_ppm(old->hres, old->vres);
  new->hres = old->hres; new->vres = old->vres;

  for (int i=0; i<new->hres*new->vres; i++) {
    new->pixel[i].r = old->pixel[i].r;
    new->pixel[i].g = old->pixel[i].g;
    new->pixel[i].b = old->pixel[i].b;
  }
...
// see .h for more details
pgm_img_t* copy_pgm(pgm_img_t* old) {
  
  if (!old) return NULL;

  pgm_img_t *new = new_pgm(old->hres, old->vres);
  new->hres = old->hres; new->vres = old->vres;

  for (int i=0; i<new->hres*new->vres; i++) {
    new->pixel[i] = old->pixel[i];
  }
